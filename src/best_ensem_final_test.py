...
parser.add_argument("--non-post", action="store_true", default=False)
...
input_path_list = [
    ...
    ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_val_10.json", 2),
    # (
    ...
    (
        "/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_val_10_day_night.json",
        1,
    ),
    (
        "/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_pretrain_val_10.json",
        1,
    ),
    (
        "/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_pretrain_val_10_sahi.json",
        2,
    ),
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_closs_val_10.json", 1),
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/test_yolor_1920_val_10.json", 1),
    ...
    # ),
    # (
    #     "/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/test_yolov6_1920_val_10_day_night.json",
    #     1,
    # ),
    # (
    #     "/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/test_yolor_1920_val_10_day_night.json",
    #     2,
    # ),
    # (
...
test_path = Path("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/CoDETR/data/yolo_all_classes_91/test_10")
...
for image_path in tqdm(list(test_path.glob("*.png"))):
    ...
    if not args.non_post:
            ...
            for j in range(len(boxes)):
                ...
                if intersect_area / area1 > 0.8 or intersect_area / area2 > 0.8:
                    if (
                        scores[i] < 0.4
                        and scores[j] < 0.4
                        or (labels[j] != labels[i] and iou > 0.5)
                    ):
                        if scores[i] > scores[j]:
                            remove_indices.add(j)
                        else:
        ...
        bus_threshold = 0.55
        truck_threshold = 0.55

        car_threshold = 0.4
        bike_threshold = 0.4
        perdestrian_threshold = 0.4
            ...
            result.append(
                {
                    ...
                    "bbox": boxes[i].tolist() if isinstance(boxes[i], np.ndarray) else boxes[i],
                    "score": score,
                }
            )
    # yolo_res = []
...
with open(result_path / "predictions_best_ensemble_test_10.json", "w+") as f:
    json.dump(result, f)
