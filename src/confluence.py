...
def main():
    ...
    img_path = "/mlcv1/Datasets/NvidiaAICityChallenge/NvidiaAIC2024/images"
    ...
    for img in tqdm(imgs):
        ...
        pred = [pred for pred in data if pred['image_id'] == img_id]
        bboxes = [p['bbox'] for p in pred]
        bboxes = [[bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]] for bbox in bboxes]
        scores = [p['score'] for p in pred]
        classes = [p['category_id'] for p in pred]

        result = confluence_nms(bboxes, scores, classes, 0.5, True)

        for cat_id, bboxes in result.items():
            for bbox in bboxes:
                new_bbox = [float(i) for i in bbox]

                assert new_bbox[2] > new_bbox[0] and new_bbox[3] > new_bbox[1], f"Invalid bbox: {new_bbox}"

                new_bbox = [new_bbox[0], new_bbox[1], new_bbox[2] - new_bbox[0], new_bbox[3] - new_bbox[1]]
