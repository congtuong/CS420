...
input_path_list = [
    ...
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_original_max_91.json", 2),
    ...
    ('/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_split_1_epoch16.json', 1),

    # CO-DETR train on day / night 20epochs
    ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/predictions_codetr_day_night.json", 1),
    ...
    ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/predictions_codetr_pretrain_finetune.json", 1),
    ...
    ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_woodscape_finetune_1280_ratio.json", 2),
    ...
    ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/predictions/predictions_fisheye_an_finetune_custom_120.json", 1),
    
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/yolor_1920_epoch80_day_night.json",1),
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/codetr_max_91_epoch20.json",1),
    ...
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/yolov6l6_1920_91.json",1),
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/yolov6l6_1920_epoch80_day_night.json",1),
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/src/temp/yolor_1920_epoch80_day_night.json",2),
    ...
    # yolov6l6 full data
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/predictions/predictions_yolov6_all_data_5class.json",1),
    ...
    # ("/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/tuongbck/sahi/results_sahi_co_dino_12g_train_square_overlap_02.json",1),
    ...
    # ('/mlcv1/WorkingSpace/Personal/tuongbck/AIC2024/baotg/classifications/source/hightest_beit_processed.json',1),
]

# NOTE: highest score, iou 0.65, conf 0.3, after conf 0.4, 7 models dau tien --> F1 = 0.273
...
def fusion_boxes(box1, box2, score1, score2):
    x = min(box1[0], box2[0])
    y = min(box1[1], box2[1])
    x2 = max(box1[2], box2[2])
    y2 = max(box1[3], box2[3])
    return [x, y, x2, y2], score1 + score2

def is_aglign(box1, box2):
    mid_box1 = [(box1[0] + box1[2]) / 2, (box1[1] + box1[3]) / 2]
    mid_box2 = [(box2[0] +  box2[2]) / 2, (box2[1] + box2[3]) / 2]
    if abs(mid_box2[1] - mid_box1[1]) < 5:
        return True
    return False
...
def load_predictions(input_path, image_path, conf_thres):
    ...
    for idx, pred in (enumerate(input_path)):
        ...
        for item in data:
            ...
            scores_dict[item["image_id"]][idx].append(
                item["score"] * weight
            )
...
for image_path in tqdm(list(test_path.glob("*.png"))):
    ...
    # distance_weights = 1
    # for i in range(len(boxes)):
    #     distance = calculate_distance([width_dict[id] / 2, height_dict[id] / 2], boxes[i])
    #     if distance > 0.7 and int(labels[i]) in [1,2,3]:
    #         scores[i] = scores[i] * (1 + distance)  *distance_weights
    #         if scores[i] > 1:
    #             scores[i] = 0.5
    ...
    for i in range(len(boxes)):
        for j in range(len(boxes)):
            ...
            #         remove_indices.add(i)
            #     continue
            # if iou > 0.01 and iou < 0.3 and int(labels[i]) == int(labels[j]) == 1:
            #     if boxes[i][0] < boxes[j][0] and area1 < area2 and scores[i] < scores[j] and boxes[i][1] > boxes[j][1] and boxes[i][3] < boxes[j][3]: 
            #         new_box, new_score = fusion_boxes(boxes[i], boxes[j], scores[i], scores[j])
            #         boxes[i] = new_box
            #         scores[i] = new_score
            #         remove_indices.add(j)
                # elif boxes[i][0] > boxes[j][0] and area1 > area2 and scores[i] > scores[j] and abs(boxes[i][1] - boxes[j][1]) < 3:
                #     new_box, new_score = fusion_boxes(boxes[i], boxes[j], scores[i], scores[j])
                #     boxes[i] = new_box
                #     scores[i] = new_score
                #     remove_indices.add(j)
            ...
            if intersect_area / area1 > 0.8 or intersect_area / area2 > 0.8:
                if scores[i] < 0.4 and scores[j] < 0.4 or (labels[j] != labels[i] and iou >0.5):
                    if scores[i] > scores[j]:
                        remove_indices.add(j)
                    else:
                        remove_indices.add(i)
    ...
    conf_list_outside = [
        ...
        bike_threshold * 0.95,
        car_threshold * 0.95,
        perdestrian_threshold * 0.95,
        truck_threshold * 0.95,
    ]

    remove_indices = set()

    # thuyentd added 26/03/2024 12:39AM
    ...
    def check_inside(boxes_t):
        center_t = (int((boxes_t[0] + boxes_t[2]) / 2), int((boxes_t[1] + boxes_t[3]) / 2))
        point = Point(*center_t)
        return ellipse.contains(point)
    ########
    for i in range(len(boxes)):
        is_inside = check_inside(boxes[i])

        if not is_inside:
            if labels[i] == 0 and scores[i] < conf_list_outside[0]:
                remove_indices.add(i)
            if labels[i] == 1 and scores[i] < conf_list_outside[1]:
                remove_indices.add(i)
            if labels[i] == 2 and scores[i] < conf_list_outside[2]:
                remove_indices.add(i)
            if labels[i] == 3 and scores[i] < conf_list_outside[3]:
                remove_indices.add(i)
            if labels[i] == 4 and scores[i] < conf_list_outside[4]:
                remove_indices.add(i)
        else:
            if labels[i] == 0 and scores[i] < conf_list[0]:
                remove_indices.add(i)
    ...
    for i in range(len(boxes)):
        if scores[i] > after_conf:
            yolo_res.append(str(int(labels[i])) + " " + " ".join([str(x) for x in xywh_2_yolo(boxes[i], width_dict[id], height_dict[id])]))
    with open(result_path / "yolo_labels" / f"{image_id}.txt", "w") as f:
        f.write("\n".join(yolo_res))

with open(result_path / "predictions_tuongbck_distance_algo_thuyentd_2.json", "w+") as f:
    json.dump(result, f)
