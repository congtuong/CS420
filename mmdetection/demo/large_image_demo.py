...
def main():
    ...
    for file in files:
        ...
        # sh = height
        # sw = width
        sh = min(args.patch_size, height)
        sw = min(args.patch_size, width)

        if args.ratio:
            r = height / width
            sh = int(args.patch_size * r)

        # if a > 1.5:
        #     sh = int(height / 2 + (overlap_height_ratio + 0.01) * height)

        # if b > 1.5:
        #     sw = int(width / 2 + (overlap_width_ratio + 0.01) * width)
        ...
        if not (Path(args.out_dir) / "preds").exists():
            (Path(args.out_dir) / "preds").mkdir()

        out_preds_file = os.path.join(args.out_dir, "preds", filename + ".json")
        ...
        # assert len(image_result) == 1, "Only support single image inference"

        res = {
            "scores": image_result.pred_instances.scores.cpu().numpy().tolist(),
            "labels": image_result.pred_instances.labels.cpu().numpy().tolist(),
            "bboxes": image_result.pred_instances.bboxes.cpu().numpy().tolist(),
        }

        with open(out_preds_file, "w+") as f:
            json.dump(res, f)
