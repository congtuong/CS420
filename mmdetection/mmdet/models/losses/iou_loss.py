...
def foo4(width, height, points):
    n_w = math.ceil((width / 2 - SPLIT / 2) / SPLIT) * 2 + 1
    n_h = math.ceil((height / 2 - SPLIT / 2) / SPLIT) * 2 + 1
    p_w = int((width - (n_w - 2) * SPLIT) / 2)
    p_h = int((height - (n_h - 2) * SPLIT) / 2)

    center_x = (points[:, 0] + points[:, 2]) / 2
    center_y = (points[:, 1] + points[:, 3]) / 2

    a = torch.floor((center_x - p_w) / SPLIT) + 1
    b = torch.floor((center_y - p_h) / SPLIT) + 1

    return (a * n_w + b).long()


def calculate_loss(width, height):
    n_w = math.ceil((width / 2 - SPLIT / 2) / SPLIT) * 2 + 1
    n_h = math.ceil((height / 2 - SPLIT / 2) / SPLIT) * 2 + 1
    p_w = int((width - (n_w - 2) * SPLIT) / 2)
    p_h = int((height - (n_h - 2) * SPLIT) / 2)

    center_ellipse = (width / 2, height / 2)
    ratio = 0.15

    ellipse = Ellipse(center_ellipse, (1 - 2 * ratio) * width, (1 - 2 * ratio) * height)
    vertices = ellipse.get_verts()
    ellipse = Polygon(vertices)

    rectangle = Polygon([(0, 0), (width, 0), (width, height), (0, height), (0, 0)])
    idx = 0
    losses = [0 for _ in range(n_w * n_h)]

    for x in range(int(p_w / 2), width, SPLIT):
        for y in range(int(p_h / 2), height, SPLIT):
            point = Point(x, y)
            ...
            old_line_length = line.length
            intersection_line = line.intersection(ellipse)
            line = scale(line, xfact=5, yfact=5)
            intersection_line_rectangle = line.intersection(rectangle)

            A = old_line_length - intersection_line.length
            B = intersection_line_rectangle.length / 2 - intersection_line.length
            C = A / B

            losses[idx] = C
            idx += 1

    return torch.Tensor(losses).cuda()
...
def giou_loss(pred: Tensor, target: Tensor, eps: float = 1e-7) -> Tensor:
    ...
    # gious cang lon -> cang match --> loss cang nho
    # phai cong them phan loss lien quan toi distance
...
def foo(points_as_np, left, right, top, bottom):

    indices = list()

    for i in range(points_as_np.shape[0]):
        points_as_np_0 = points_as_np[i]
        index = np.where(
            (points_as_np_0[0] > left)
            * (points_as_np_0[0] <= right)
            * (points_as_np_0[1] > top)
            * (points_as_np_0[1] <= bottom)
        )
        if len(index[0]) == 0:
            continue
        index = index[0][0]
...
@weighted_loss
def giou_loss_bao(
    pred: Tensor, target: Tensor, eps: float = 1e-7, img_shape=None, **kwargs
) -> Tensor:
    r"""`Generalized Intersection over Union: A Metric and A Loss for Bounding
    Box Regression <https://arxiv.org/abs/1902.09630>`_.
    ...
    width, height = img_shape[0]

    if (width, height) not in data_loss:
        loss_indices = calculate_loss(width, height)
        data_loss[(width, height)] = loss_indices
    else:
        loss_indices = data_loss[(width, height)]

    indices = foo4(width, height, pred)

    mask = torch.isin(indices, torch.arange(loss_indices.shape[0]).cuda())
    indices = torch.where(mask, indices, torch.zeros_like(indices))
    loss = (1 - gious) * 1.2 ** (loss_indices[indices])
      
    # except:
    #     print(f"{loss_indices =}")
    #     print(f"{indices =}")
    #     print(f"len(loss_indices) = {len(loss_indices)}")
    #     print(f"{indices > len(loss_indices) =}")

    #     for i in range(len(indices)):
    #         print(f"{indices[i] = }, {pred[i] =}")
...
class GIoULossBao(nn.Module):
    ...
    def __init__(
        self, eps: float = 1e-6, reduction: str = "mean", loss_weight: float = 1.0, use_new: bool = True
    ) -> None:
        super().__init__()
        self.eps = eps
        self.reduction = reduction
    ...
    def forward(
        ...
        avg_factor: Optional[int] = None,
        reduction_override: Optional[str] = None,
        img_metas: Optional[Dict[str, Any]] = None,
        **kwargs,
    ) -> Tensor:
        """Forward function.
        ...
        # assert img_metas is not None, print(kwargs)

        if self.use_new:
            img_shape = None
            batch_data_samples = kwargs.get("batch_data_samples")

            if img_metas is not None:
                img_shape = [x["img_shape"] for x in img_metas]

            if batch_data_samples is not None:
                img_shape = [x.img_shape for x in batch_data_samples]
        ...
        if self.use_new:
            loss = self.loss_weight * giou_loss_bao(
                pred,
                target,
                weight,
                eps=self.eps,
